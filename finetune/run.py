...
from utils.optim import create_optimizer, get_parameter_groups, LayerDecayValueAssigner
from finetune.engine import train_one_epoch, validation_one_epoch, final_test, merge
...
def get_args():
    ...
    parser.add_argument('--model_key', default='model|module|model_state', type=str)
...
def main(args, ds_init):
    ...
    if args.finetune:
        ...
        for key in all_keys:
            ...
            elif key.startswith('encoder_q.encoder.model.'):
                new_dict[key[24:]] = checkpoint_model[key]
            else:
        ...
        for i,key in enumerate(fc_keys):
            if key in checkpoint_model:
                new_dict[head_keys[i]] = checkpoint_model[key]
        ...
        if 'pos_embed' in checkpoint_model:
            ...
            if len(checkpoint_model['patch_embed.proj.weight'].shape) != len(model.state_dict()['patch_embed.proj.weight'].shape):
                from einops import repeat
                w = checkpoint_model['patch_embed.proj.weight']
                w = repeat(w, 'c1 c2 h w -> c1 c2 t h w', t=patch_size[0])
                checkpoint_model['patch_embed.proj.weight'] = w / patch_size[0]
    ...
    # final test
    print(f"Start final testing!")
    preds_floder = os.path.join(args.output_dir, 'pred')
    os.makedirs(preds_floder, exist_ok=True)
    preds_file = os.path.join(preds_floder, str(global_rank) + '.txt')
    test_stats = final_test(data_loader_test, model, device, preds_file)
    torch.distributed.barrier()
    if global_rank == 0:
        print("Start merging results...")
        final_top1 ,final_top5 = merge(preds_floder, num_tasks)
