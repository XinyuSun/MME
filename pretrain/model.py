...
from finetune.model import Block, _cfg, PatchEmbed, get_sinusoid_encoding_table
...
if __name__ == '__main__':
    ...
    model = pretrain_videomae_base_patch16_224(
        ...
        decoder_num=2,
        decoder_embed_dim=384,
        decoder_num_classes=[9*3*4, 6*2*4],
        decoder_act=[nn.Sigmoid, nn.Tanh],
    )
    
    dummy_input = torch.zeros([1,3,16,224,224])
    bool_masked_pos = torch.concat([torch.ones(1097), torch.zeros(1568-1097)]).to(torch.bool)
    flops = FlopCountAnalysis(model, (dummy_input, bool_masked_pos[None]))
    print(f'FLOPs for a individual video input: {flops.total() // 1e9}G')